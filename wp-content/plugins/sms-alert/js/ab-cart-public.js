        ...
        function () {
            ...
            function getCheckoutData()
            {
                ...
                if(typeof sa_otp_settings !=  'undefined' && sa_otp_settings['show_countrycode']=='on' ) {
                    var mob_field = $(".checkout [name=billing_phone]:hidden,.checkout [name=billing-phone]:hidden");
                } else{
                    var mob_field = $(".checkout [name=billing_phone],.checkout [name=billing-phone]");
                }
            
                if(mob_field.length > 0 ) { //If phone number exists

                    var ab_cart_phone     = mob_field.val();
                    var ab_cart_email     = $("#billing_email,#billing-email").val();
                    ...
                    if (ab_cart_phone.length >= 1) { //Checking if the phone number is longer than 1 digit
                        //If Phone valid
                        var ab_cart_name                 = $("#billing_first_name,#billing-first_name").val();
                        var ab_cart_nonce                 = $("#smsalert_abcart_nonce").val();
                        var ab_cart_surname             = $("#billing_last_name,#billing-last_name").val();
                        var ab_cart_phone                 = mob_field.val();
                        var ab_cart_country             = $("#billing_country,#billing-country").val();
                        var ab_cart_city                 = $("#billing_city,#billing-city").val();

                        //Other fields used for "Remember user input" function
                        var ab_cart_billing_company     = $("#billing_company,#billing-company").val();
                        var ab_cart_billing_address_1     = $("#billing_address_1,#billing-address_1").val();
                        var ab_cart_billing_address_2     = $("#billing_address_2,#billing-address_2").val();
                        var ab_cart_billing_state         = $("#billing_state,#billing-state").val();
                        var ab_cart_billing_postcode     = $("#billing_postcode,#billing-postcode").val();
                        var ab_cart_shipping_first_name = $("#shipping_first_name,#shipping-first_name").val();
                        var ab_cart_shipping_last_name     = $("#shipping_last_name,#shipping-last_name").val();
                        var ab_cart_shipping_company     = $("#shipping_company,#shipping-company").val();
                        var ab_cart_shipping_country     = $("#shipping_country,#shipping-country").val();
                        var ab_cart_shipping_address_1     = $("#shipping_address_1,#shipping-address_1").val();
                        var ab_cart_shipping_address_2     = $("#shipping_address_2,#shipping-address_2").val();
                        var ab_cart_shipping_city         = $("#shipping_city,#shipping-city").val();
                        var ab_cart_shipping_state         = $("#shipping_state,#shipping-state").val();
                        var ab_cart_shipping_postcode     = $("#shipping_postcode,#shipping-postcode").val();
