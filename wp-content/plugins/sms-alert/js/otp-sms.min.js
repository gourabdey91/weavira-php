...
function add_smsalert_button(submit_selector,phone_selector,unique_id,button_text='')
{
    if(!$sa(submit_selector).hasClass("sa-default-btn-hide") && ($sa(phone_selector).attr('type') !== 'hidden' || $sa(phone_selector).parents("form").hasClass('woocommerce-post-checkout-form'))) {
		...
		var otpButton = button.first().clone();
		otpButton
        .insertAfter(button.first())
        .addClass("sa-otp-btn-init smsalert_otp_btn_submit")
        .attr("id", "sa_verify_" + unique_id)
        .attr("name", "sa_verify_" + unique_id)
        .removeAttr("onclick")
        .removeAttr("onkeypress")
        .removeClass("sa-default-btn-hide cod-default-btn-hide");
		
		if($sa(phone_selector).prop('nodeName') === 'INPUT')
		{
          $sa(phone_selector).addClass("phone-valid");
		}
        if(button.first().is("button")) {
			var text = (button_text !== "")
            ? button_text
            : button.first().text();
            otpButton.html("<span class='button__text'>" + text + "</span>");
        }
    }
}
