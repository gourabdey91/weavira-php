...
<header class="ck-header">
  <a href="{{ home_url('/') }}" class="ck-header-brand">
    <img src="{!! get_field('arc_option_logo', 'option') !!}" alt="{{ $siteName }}" class="ck-header-logo" />
  </a>
...
<div class="ck-progress" role="list" aria-label="Checkout steps">
  ...
  <div class="ck-step @if($skipStep1) ck-step--active @endif" data-step="2" role="listitem">
    <div class="ck-step-num" @if($skipStep1) aria-current="step" @endif>2</div>
...
<main class="ck-layout">
  ...
  <div class="ck-main">
    ...
    {{-- STEP 1: Continue — content only renders for guests, and deliberately
         lives OUTSIDE <form id="ck-checkout-form"> below. The SMS Alert
         plugin's [sa_loginwithotp] shortcode renders its own <form>;
         nested <form> elements are invalid HTML, and the browser's parser
         closes our real checkout form at the first nested </form> it
         hits — every field after that point (shipping, payment, the
         place-order nonce/button) ends up outside #ck-checkout-form and
         never gets submitted. Keeping Step 1 out of the form entirely
         avoids that. --}}
    ...
    <form name="checkout" method="post" class="checkout woocommerce-checkout" action="{{ wc_get_checkout_url() }}" enctype="multipart/form-data" id="ck-checkout-form">

      {{-- The visible shipping_country/billing_country fields are
           disabled/locked to India (see @php block above) — disabled
           fields don't submit, so these hidden twins carry the actual
           values, regardless of which one ends up being the "real"
           billing address (mirrored delivery, or a distinct GST address —
           both are always India). --}}
      <input type="hidden" name="shipping_country" value="IN" />
      <input type="hidden" name="billing_country" value="IN" />

      {{-- STEP 2: Delivery Address --}}
      <button class="ck-accordion" type="button" aria-expanded="{{ $skipStep1 ? 'true' : 'false' }}" aria-controls="ck-panel-2" data-step-toggle="2">
      ...
      <div class="ck-accordion-panel" id="ck-panel-2" data-step="2" @if(!$skipStep1) hidden @endif>
          ...
          @if($hasShippingCards)
            <div class="ck-address-cards" id="ck-shipping-address-cards" role="radiogroup" aria-label="Use a saved address">
              @foreach($savedShippingAddresses as $saved)
                ...
                ?>
                {{-- get_formatted_address() already includes the name
                     (first_name + last_name) as its own first line — a
                     separate name line here would just repeat it. --}}
                <div class="ck-address-card @if($shippingDefault && $shippingDefault['id'] === $saved['id']) is-selected @endif" role="button" tabindex="0" data-address-id="{{ $saved['id'] }}">
                  <span class="ck-address-card-icon"><i data-lucide="{{ $cardIcon }}" aria-hidden="true"></i></span>
                  <div class="ck-address-card-body">
                    <div class="ck-address-card-head">
                      <span class="ck-address-card-label">{{ $cardLabel }}</span>
                      @if(!empty($saved['is_default']))
                    ...
                    @if(!empty($saved['phone']))
                      <span class="ck-address-card-phone"><strong>Mobile Number:</strong> {{ $saved['phone'] }}</span>
                    @endif
                  </div>
            ...
            {{-- Add/Edit popup — the field grid above stays permanently
                 hidden once any address is saved (see #ck-shipping-field-grid
                 below); "+ Add New Address" and each card's "Edit" button
                 both open this same modal instead (see weavira.js). Saves
                 via the same weavira_save_checkout_shipping_address AJAX
                 action as the zero-address auto-save, with an address_id
                 when editing; the page reloads on success so the
                 server-rendered cards/defaults stay authoritative. --}}
        ...
        <div class="ck-gst-block">
          ...
          <div class="ck-gst-fields" id="ck-gst-fields" hidden>
            @if(count($savedBillingAddresses ?? []) > 1)
              <p class="form-row form-row-wide">
                ...
                <select id="wv-billing-address-select" class="woocommerce-Input woocommerce-Input--select input-select">
                  ...
                  @foreach($savedBillingAddresses as $saved)
                    <option value="{{ $saved['id'] }}">{{ $saved['label'] }}</option>
                  @endforeach
                </select>
              </p>
            @endif
          ...
          @foreach($cartItems as $item)
            <div class="ck-gift-item" data-cart-item-key="{{ $item['key'] }}">
              ...
              <div class="wv-gift-fields ck-gift-form" data-gift-fields @if(!$item['gift']) hidden @endif>
                ...
                <div class="wv-gift-field">
                  <label for="ck-gift-occasion-{{ $loop->index }}">Occasion</label>
                  <select id="ck-gift-occasion-{{ $loop->index }}" class="wv-gift-occasion">
                ...
                <div class="wv-gift-field wv-gift-field--full">
                  <label for="ck-gift-recipient-{{ $loop->index }}">Gift Recipient Name</label>
                  <input type="text" id="ck-gift-recipient-{{ $loop->index }}" class="wv-gift-recipient" value="{{ $item['gift']['recipient_name'] ?? '' }}" placeholder="Enter recipient&rsquo;s name" maxlength="80" />
                </div>
      ...
      <div class="ck-accordion-panel" id="ck-panel-{{ $reviewStepNum }}" data-step="{{ $reviewStepNum }}" hidden>
  ...
  <aside class="ck-summary" aria-label="Order Summary">
    ...
    <div class="ck-summary-head">
      <div>
        ...
        <p class="ck-summary-count">{{ $cartCount }} Item{{ $cartCount === 1 ? '' : 's' }}</p>
      </div>
      <a href="{{ wc_get_cart_url() }}" class="ck-edit-cart">Edit Cart</a>
    </div>
      ...
      @foreach($cartItems as $item)
        <div class="ck-order-item">
          ...
          <div class="ck-order-info">
            ...
            <p class="ck-order-meta">Qty: {{ $item['quantity'] }} @if($item['gift'])<span class="ck-dot" aria-hidden="true">&bull;</span> Gift @endif</p>
          </div>
