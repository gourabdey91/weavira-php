...
<div class="w-pdp-info">
  ...
  @if($badgeLabel)
    <span class="product-badge">{{ strtoupper($badgeLabel) }}</span>
  @endif

  <h1 class="w-pdp-title">{{ $product->get_name() }}</h1>

  @if($shortDescription)
    <p class="w-pdp-description">{!! $shortDescription !!}</p>
  @endif

  @if(!empty($attrs['weave']) || !empty($attrs['material']) || !empty($attrs['blouse-piece']))
      ...
      @if(!empty($attrs['weave']))
        <div class="w-pdp-specs-item">
          ...
          <span class="w-pdp-specs-text">
            ...
            <span class="w-pdp-specs-value">{{ $attrs['weave'] }}</span>
          </span>
        </div>
      @endif
      ...
      @if(!empty($attrs['material']))
        <div class="w-pdp-specs-item">
          ...
          <span class="w-pdp-specs-text">
            ...
            <span class="w-pdp-specs-value">{{ $attrs['material'] }}</span>
          </span>
        </div>
      @endif
      ...
      @if(!empty($attrs['blouse-piece']))
        <div class="w-pdp-specs-item">
          ...
          <span class="w-pdp-specs-text">
            ...
            <span class="w-pdp-specs-value">{{ $attrs['blouse-piece'] === 'Yes' ? 'Included' : 'Not included' }}</span>
          </span>
        </div>
      @endif
    </div>
        ...
        <div class="ppbox-facts">
          @foreach($craftFacts as $fact)
            <div class="ppbox-fact">
              ...
              <span class="ppbox-fact-text">{{ $fact }}</span>
            </div>
          @endforeach
        </div>
      @endif
